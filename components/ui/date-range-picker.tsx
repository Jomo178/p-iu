      ...
      <PopoverTrigger asChild>
        <Button size={"lg"} variant="outline" className={cn(className)}>
          <div className="text-right">
            <div className="py-1">
              <div>{`${formatDate(range.from, locale)} ${range.from.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}${
                range.to != null
                  ? " - " +
                    formatDate(range.to, locale) +
                    " " +
                    range.to.toLocaleTimeString(locale, {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : ""
            ...
            {rangeCompare != null && (
              <div className="-mt-1 text-xs opacity-60">
                <>
                  ...
                  {rangeCompare.from.toLocaleTimeString(locale, {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                  {rangeCompare.to != null
                    ? ` - ${formatDate(rangeCompare.to, locale)} ${rangeCompare.to.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}`
                    : ""}
                </>
              </div>
            )}
              ...
              <div className="flex flex-col items-center justify-center gap-2 px-3 pb-4 lg:flex-row lg:items-start lg:pb-0">
                <div className="flex flex-row gap-2">
                  <div className="flex flex-col items-center gap-2 md:flex-row">
                    ...
                    <Select
                      value={
                        range.from.getHours().toString().padStart(2, "0") +
                        ":" +
                        range.from.getMinutes().toString().padStart(2, "0")
                      }
                      onValueChange={(e) => {
                        const [hours, minutes] = e.split(":");
                        const newFromDate = new Date(range.from.getTime());
                        ...
                        newToDate.setHours(parseInt(hours), parseInt(minutes));
                  ...
                  >
                    <SelectTrigger className="mx-auto h-full w-[180px]">
...
const DateInput: React.FC<DateInputProps> = ({
  ...
  return (
    <div className="flex h-9 items-center rounded-lg border px-1 text-sm">
      <input
        type="text"
        ref={monthRef}
        max={12}
